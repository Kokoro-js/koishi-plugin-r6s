import type { Canvas } from "koishi-plugin-skia-canvas";
import path from "path";
import { User, Ranked, Profile } from "./types";

export async function rankPicture(
  canvasSerivce: Canvas,
  target: User,
  profile: Profile,
  rankInfo: Ranked,
  status: "online" | "away" | "dnd" | "offline",
) {
  const canvas = canvasSerivce.createCanvas(350, 500);
  ...
  const rank = Buffer.from(
    "Z2l0aHViIGFoZGc2L2tvaXNoaS1wbHVnaW4tcjZzIA==",
    "base64",
  ).toString();
  // top rectangle
  ...
  context.font = `20px LXGW WenKai Lite`;
  ...
  if (rankInfo.topRankPosition > 0)
    context.fillText(`冠军排名：${rankInfo.topRankPosition}`, 130, 350);
  context.fillText(
    `赛季：${profile.season.shorthand} - ${profile.season.id}`,
    130,
    375,
  );
  // create image
  const svgRankPath = path.join(
    __dirname,
    "assets",
    "rank",
    `${rankInfo.rank.slug}.svg`,
  );
  const svgTopRankPath = path.join(
    __dirname,
    "assets",
    "rank",
    `${rankInfo.maxRank.slug}.svg`,
  );
  try {
    const rank = await canvasSerivce.loadImage(svgRankPath);
    const maxRank = await canvasSerivce.loadImage(svgTopRankPath);
    const avast = await canvasSerivce.loadImage(target.avatars["146"]);
